...
Return STRICT JSON: {"title": str, "tagline": str, "kicker": str, "steps": [ {"do": "click|hover|scroll|wait|cue", "text"?: str, "note"?: str, "times"?: int, "ms"?: int} ], "narration": str, "beats": [ {"say": str, "at": str} ], "thumbHeadline": str, "thumbSub": str }
...
- beats = the voiceover broken into timed lines, ONE per meaningful moment, IN THE SAME ORDER as steps. The first beat has "at":"start" (intro line). Then ONE beat for EACH click/hover step whose "at" is EXACTLY that step's target "text". Optionally a final beat with "at":"end" (closing line). Each "say" is ONE short, simple sentence (max ~14 words) describing what is happening at that exact moment ("Open Audits to see your site's health.", "Here the dashboard shows your key scores."). The beats together read as the full narration.
Rules:
